...
pub fn serialize_field_element(field_element: ConstraintF) -> Result<Vec<u8>> {
    ...
    field_element
        .serialize_compressed(&mut bytes_field_element)         // CHANGED FROM SERIALIZE
...
pub fn deserialize_field_element(bytes_field_element: Vec<u8>) -> Result<ConstraintF> {
    ConstraintF::deserialize_compressed(&mut bytes_field_element.as_slice())        // CHNAGED FROM DESERIALIZE
